...
def cp_loglikelihood_proj_flux(params,u,v,wavel,proj_t3data,proj_t3err,proj,model='constant'):
...
def chi2_grid(everything):
    ...
    for ix in range(ys.size):
        for k,con in enumerate(everything['cons']):
            ...
            if projected:
                chi2[ix,k] = - 2*cp_loglikelihood_proj_flux(params,cpo.u,cpo.v,cpo.wavel,
                    cpo.proj_t3data,cpo.proj_t3err,cpo.proj)
            else:
                chi2[ix,k] = - 2*cp_loglikelihood(params,cpo.u,cpo.v,cpo.wavel,
                    cpo.t3data,cpo.t3err)
...
#    cons = cmin  + (cmax-cmin)  * np.linspace(0,1,ncon)
    ...
    if threads ==0:
        ...
        for ix,x in enumerate(xys):
            everything={'x':x,'cons':cons,'ys':xys,'cpo':cpo,'ix':ix,'projected':projected}
        ...
        all_vars=[]
        for ix in range(nxy):
            everything={'x':xys[ix],'cons':cons,'ys':xys,'cpo':cpo,'ix':ix,'projected':projected}
    ...
    for ix,n in enumerate(names):
        ...
        plt.imshow(plots[ix],extent=[np.amin(xys),np.amax(xys),np.amin(xys),np.amax(xys)],aspect='auto',cmap=cmap)
        plt.colorbar()
        plt.ylabel('Dec (mas)')
        plt.xlabel('RA (mas)')
