...
class OI_ARRAY(object):
    ...
    def __init__(self, frame, arrxyz, stations=()):
        ...
        for station in stations:
            tel_name, sta_name, sta_index, diameter, staxyz = station['tel_name'], station[
                'sta_name'], station['sta_index'], station['diameter'], station['staxyz']
            self.station = np.append(self.station, OI_STATION(
                tel_name=tel_name, sta_name=sta_name, diameter=diameter, staxyz=staxyz))
        ...
        if new.header != None:
            if other.header != None:
                ...
                try:
                    ...
                    warnings.warn(
                        'Warning: Keeping FITS header from first oifits object', UserWarning)
    ...
    def isvalid(self):
        ...
        if not self.wavelength:
            ...
            for wavelength in self.wavelength.values():
                if len(wavelength.eff_wave) != len(wavelength.eff_band):
                    errors.append(
                        "eff_wave and eff_band are of different lengths for wavelength table '%s'" % key)
        ...
        for vis in self.vis:
            ...
            if (len(vis.visamp) != nwave) or (len(vis.visamperr) != nwave) or (len(vis.visphi) != nwave) or (len(vis.visphierr) != nwave) or (len(vis.flag) != nwave):
                errors.append(
                    "Data size mismatch for visibility measurement 0x%x (wavelength table has a length of %d)" % (id(vis), nwave))
        for vis2 in self.vis2:
            ...
            if (len(vis2.vis2data) != nwave) or (len(vis2.vis2err) != nwave) or (len(vis2.flag) != nwave):
                errors.append(
                    "Data size mismatch for visibility^2 measurement 0x%x (wavelength table has a length of %d)" % (id(vis), nwave))
        for t3 in self.t3:
        ...
        if errors:
            print("*** %d ERROR%s:" %
                  (len(errors), _plurals(len(errors)).upper()))
            for error in errors:
                print('  ' + error)
    ...
    def isconsistent(self):
        ...
        for vis in self.vis:
            if vis.array and (vis.array not in self.array.values()):
                print(
                    'A visibility measurement (0x%x) refers to an array which is not inside the main oifits object.' % id(vis))
                return False
            if ((vis.station[0] and (vis.station[0] not in vis.array.station)) or
                    (vis.station[1] and (vis.station[1] not in vis.array.station))):
                print(
                    'A visibility measurement (0x%x) refers to a station which is not inside the main oifits object.' % id(vis))
                return False
            ...
            if vis.target not in self.target:
                print(
                    'A visibility measurement (0x%x) refers to a target which is not inside the main oifits object.' % id(vis))
                return False

        for vis2 in self.vis2:
            if vis2.array and (vis2.array not in self.array.values()):
                print(
                    'A visibility^2 measurement (0x%x) refers to an array which is not inside the main oifits object.' % id(vis2))
                return False
            if ((vis2.station[0] and (vis2.station[0] not in vis2.array.station)) or
                ...
                print(
                    'A visibility^2 measurement (0x%x) refers to a station which is not inside the main oifits object.' % id(vis))
                return False
            ...
            if vis2.target not in self.target:
                print(
                    'A visibility^2 measurement (0x%x) refers to a target which is not inside the main oifits object.' % id(vis2))
                return False

        for t3 in self.t3:
            if t3.array and (t3.array not in self.array.values()):
                print(
                    'A closure phase measurement (0x%x) refers to an array which is not inside the main oifits object.' % id(t3))
                return False
            if ((t3.station[0] and (t3.station[0] not in t3.array.station)) or
                ...
                print(
                    'A closure phase measurement (0x%x) refers to a station which is not inside the main oifits object.' % id(t3))
                return False
            ...
            if t3.target not in self.target:
                print(
                    'A closure phase measurement (0x%x) refers to a target which is not inside the main oifits object.' % id(t3))
                return False
    ...
    def save(self, filename):
        ...
        for insname, wavelength in self.wavelength.items():
            ...
            hdu = pyfits.BinTableHDU.from_columns(pyfits.ColDefs((
                pyfits.Column(name='EFF_WAVE', format='1E',
                              unit='METERS', array=wavelength.eff_wave),
                pyfits.Column(name='EFF_BAND', format='1E',
                              unit='METERS', array=wavelength.eff_band)
            )))
        ...
        if self.target.size:
            ...
            hdu = pyfits.BinTableHDU.from_columns(pyfits.ColDefs((
                ...
                pyfits.Column(name='RAEP0', format='1D',
                              unit='DEGREES', array=raep0),
                pyfits.Column(name='DECEP0', format='1D',
                              unit='DEGREES', array=decep0),
                pyfits.Column(name='EQUINOX', format='1E',
                              unit='YEARS', array=equinox),
                pyfits.Column(name='RA_ERR', format='1D',
                              unit='DEGREES', array=ra_err),
                pyfits.Column(name='DEC_ERR', format='1D',
                              unit='DEGREES', array=dec_err),
                pyfits.Column(name='SYSVEL', format='1D',
                              unit='M/S', array=sysvel),
                pyfits.Column(name='VELTYP', format='8A', array=veltyp),
                pyfits.Column(name='VELDEF', format='8A', array=veldef),
                pyfits.Column(name='PMRA', format='1D',
                              unit='DEG/YR', array=pmra),
                pyfits.Column(name='PMDEC', format='1D',
                              unit='DEG/YR', array=pmdec),
                pyfits.Column(name='PMRA_ERR', format='1D',
                              unit='DEG/YR', array=pmra_err),
                pyfits.Column(name='PMDEC_ERR', format='1D',
                              unit='DEG/YR', array=pmdec_err),
                pyfits.Column(name='PARALLAX', format='1E',
                              unit='DEGREES', array=parallax),
                pyfits.Column(name='PARA_ERR', format='1E',
                              unit='DEGREES', array=para_err),
        ...
        for arrname, array in self.array.items():
            ...
            if array.station.size:
                ...
                hdu = pyfits.BinTableHDU.from_columns(pyfits.ColDefs((
                    pyfits.Column(name='TEL_NAME',
                                  format='16A', array=tel_name),
                    pyfits.Column(name='STA_NAME',
                                  format='16A', array=sta_name),
                    pyfits.Column(name='STA_INDEX',
                                  format='1I', array=sta_index),
                    pyfits.Column(name='DIAMETER', unit='METERS',
                                  format='1E', array=diameter),
                    pyfits.Column(name='STAXYZ', unit='METERS',
                                  format='3D', array=staxyz)
                )))
        ...
        if self.vis.size:
            ...
            for key in tables.keys():
                ...
                hdu = pyfits.BinTableHDU.from_columns(pyfits.ColDefs([
                    pyfits.Column(name='TARGET_ID', format='1I',
                                  array=data['target_id']),
                    pyfits.Column(name='TIME', format='1D',
                                  unit='SECONDS', array=data['time']),
                    pyfits.Column(name='MJD', unit='DAY',
                                  format='1D', array=data['mjd']),
                    pyfits.Column(name='INT_TIME', format='1D',
                                  unit='SECONDS', array=data['int_time']),
                    pyfits.Column(name='VISAMP', format='%dD' %
                                  nwave, array=data['visamp']),
                    pyfits.Column(name='VISAMPERR', format='%dD' %
                                  nwave, array=data['visamperr']),
                    pyfits.Column(name='VISPHI', unit='DEGREES',
                                  format='%dD' % nwave, array=data['visphi']),
                    pyfits.Column(name='VISPHIERR', unit='DEGREES',
                                  format='%dD' % nwave, array=data['visphierr']),
                    pyfits.Column(name='CFLUX', format='%dD' %
                                  nwave, array=data['cflux']),
                    pyfits.Column(name='CFLUXERR', format='%dD' %
                                  nwave, array=data['cfluxerr']),
                    pyfits.Column(name='UCOORD', format='1D',
                                  unit='METERS', array=data['ucoord']),
                    pyfits.Column(name='VCOORD', format='1D',
                                  unit='METERS', array=data['vcoord']),
                    pyfits.Column(name='STA_INDEX', format='2I',
        ...
        if self.vis2.size:
            ...
            for key in tables.keys():
                ...
                hdu = pyfits.BinTableHDU.from_columns(pyfits.ColDefs([
                    ...
                    pyfits.Column(name='TIME', format='1D',
                                  unit='SECONDS', array=data['time']),
                    pyfits.Column(name='MJD', format='1D',
                                  unit='DAY', array=data['mjd']),
                    pyfits.Column(name='INT_TIME', format='1D',
                                  unit='SECONDS', array=data['int_time']),
                    pyfits.Column(name='VIS2DATA', format='%dD' %
                                  nwave, array=data['vis2data']),
                    pyfits.Column(name='VIS2ERR', format='%dD' %
                    ...
                    pyfits.Column(name='STA_INDEX', format='2I',
                                  array=data['sta_index'], null=-1),
                    pyfits.Column(name='FLAG', format='%dL' %
        ...
        if self.t3.size:
            ...
            for key in tables.keys():
                ...
                hdu = pyfits.BinTableHDU.from_columns(pyfits.ColDefs((
                    ...
                    pyfits.Column(name='INT_TIME', format='1D',
                                  unit='SECONDS', array=data['int_time']),
                    pyfits.Column(name='T3AMP', format='%dD' %
                                  nwave, array=data['t3amp']),
                    pyfits.Column(name='T3AMPERR', format='%dD' %
                                  nwave, array=data['t3amperr']),
                    pyfits.Column(name='T3PHI', format='%dD' %
                                  nwave, unit='DEGREES', array=data['t3phi']),
                    pyfits.Column(name='T3PHIERR', format='%dD' %
                                  nwave, unit='DEGREES', array=data['t3phierr']),
                    pyfits.Column(name='U1COORD', format='1D',
                                  unit='METERS', array=data['u1coord']),
                    pyfits.Column(name='V1COORD', format='1D',
                                  unit='METERS', array=data['v1coord']),
                    pyfits.Column(name='U2COORD', format='1D',
                                  unit='METERS', array=data['u2coord']),
                    pyfits.Column(name='V2COORD', format='1D',
                                  unit='METERS', array=data['v2coord']),
                    pyfits.Column(name='STA_INDEX', format='3I',
                                  array=data['sta_index'], null=-1),
                    pyfits.Column(name='FLAG', format='%dL' %
                                  nwave, array=data['flag'])
                )))
        ...
        if hdu.name == 'OI_VIS':
            for row in data:
                if 'T' in header['DATE-OBS']:
                    warnings.warn(
                        'Warning: DATE-OBS contains a timestamp, which is contradictory to the OIFITS2 standard', UserWarning)
