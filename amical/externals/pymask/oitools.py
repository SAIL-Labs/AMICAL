...
def match_wavelength(template, oifitsobj):
    ...
    for tvis, vis in zip(template.vis, oifitsobj.vis):
        ...
        vis._visamp = np.interp(
            tvis.wavelength.eff_wave, vis.wavelength.eff_wave[idx], vis._visamp[idx]
        )
        vis._visamperr = np.interp(
            tvis.wavelength.eff_wave, vis.wavelength.eff_wave[idx], vis._visamperr[idx]
        )
        vis._visphi = np.interp(
            tvis.wavelength.eff_wave, vis.wavelength.eff_wave[idx], vis._visphi[idx]
        )
        vis._visphierr = np.interp(
            tvis.wavelength.eff_wave, vis.wavelength.eff_wave[idx], vis._visphierr[idx]
        )
        if vis._cflux != None:
            vis._cflux = np.interp(
                tvis.wavelength.eff_wave, vis.wavelength.eff_wave[idx], vis._cflux[idx]
            )
        if vis._cfluxerr != None:
            vis._visamperr = np.interp(
                tvis.wavelength.eff_wave,
                vis.wavelength.eff_wave[idx],
                vis._cfluxerr[idx],
            )
...
def plot_full_visamp_vs_spatfreq(source, pmin=None, pmax=None, showerror=False):
    ...
    for vis in source.vis:
        ...
        if showerror:
            ax.errorbar(
                ...
                vis.visamp,
                vis.visamperr,
                color=cmap((posangle - pmin) / (pmax - pmin)),
            )
        else:
    ...
    cbarax = fig.add_axes(
        (position[0, 0], 0.9, position[1, 0] - position[0, 0], 0.05), frameon=False
    )
    cbarax.imshow(
        [np.linspace(0, 1, 100)], cmap=cmap, aspect="auto", extent=(pmin, pmax, 0, 1)
    )
...
def plot_visamp_vs_spatfreq(
    ...
    ax.text(
        0.98,
        0.98,
        r"$\lambda=%.1f \mu$m" % (wave[waveidx].mean()),
        ha="right",
        va="top",
...
def plot_gaussian_widths(source, waveidx=100, legend=False):
    ...
    for vis in source.vis:
        ...
        fwhm_upper = get_gaussian_width_from_visibility(
            baseline / vis.wavelength.eff_wave[waveidx],
            vis.visamp[waveidx] - vis.visamperr[waveidx],
        )
        fwhm_lower = get_gaussian_width_from_visibility(
            baseline / vis.wavelength.eff_wave[waveidx],
            vis.visamp[waveidx] + vis.visamperr[waveidx],
        )
...
def plot_visamp_map(oifitsobj, width=0.1, height=0.1, colorcoding=None):
    ...
    for vis in oifitsobj.vis:
        ...
        print(B, pa)
        ax = fig.add_axes(
            (B / 80.0 - width / 2.0, pa / 180.0 - height / 2.0, width, height),
            axis_bgcolor=(0, 0, 0, 0),
        )
        if colorcoding:
...
def plot_phases(oidata, uvplot=False, legend=False):
    ...
    for vis in oidata.vis:
        ...
        for bounds in ranges:
            ...
            rect = matplotlib.patches.Rectangle(
                (w1, -180), w2 - w1, 360, color="lightblue"
            )
            ax1.add_patch(rect)
    ...
    for vis in oidata.vis:
        ...
        xmin = np.amin(
            np.append(1e6 * vis.wavelength.eff_wave[np.where(vis.flag == False)], xmax)
        )
        xmax = np.amax(
            np.append(1e6 * vis.wavelength.eff_wave[np.where(vis.flag == False)], xmin)
        )
        if uvplot:
    ...
    if legend and uvplot:
        ax2.legend(prop={"size": 10}, numpoints=1)
...
def plot_visibilities(oidata, uvplot=False, legend=False, ploterror=False):
    ...
    for vis in oidata.vis:
        ...
        for bounds in ranges:
            ...
            w2 = vis.wavelength.eff_wave[bounds[1]] * 1e6
            rect = matplotlib.patches.Rectangle(
                (w1, 0), w2 - w1, 999, color="lightblue"
            )
            ax1.add_patch(rect)
    ...
    for vis in oidata.vis:
        ...
        try:
            if ploterror:
                ...
                line = ax1.plot(
                    1e6 * vis.wavelength.eff_wave, vis.visamp, label=label, color=color
                )
        except:
            if ploterror:
                line = ax1.errorbar(
...
def plot_cflux(oidata, uvplot=False, legend=False, ploterror=False):
    ...
    for vis in oidata.vis:
        ...
        if ploterror:
            ...
            line = ax1.plot(
                1e6 * vis.wavelength.eff_wave, vis.cflux, label=label, color=color
            )
...
def plot_vis2(oidata, uvplot=False, legend=False, ploterror=False):
    ...
    for vis in oidata.vis2:
        ...
        if ploterror:
            ...
            line = ax1.plot(
                1e6 * vis.wavelength.eff_wave, vis.vis2data, label=label, color=color
            )
...
def print_vis(vis):
    ...
    print(
        ...
        % (
            vis.target.target,
            vis.station[0].sta_name + vis.station[1].sta_name,
            vis.timeobs,
        )
    )
    ...
    for i in idx:
        print(
            "%6.3f %8.4f %8.4f"
            % (1e6 * vis.wavelength.eff_wave[i], vis.visamp[i], vis.visamperr[i])
        )
...
def plot_array(array, visibilities=None):
    ...
    transOffset = offset_copy(
        ax.transData, fig=ax.figure, x=0.0, y=-0.10, units="inches"
    )
    ...
    if visibilities:
        for vis in visibilities:
            x = np.array(
                [
                    np.inner(vis.station[0].staxyz, east),
                    np.inner(vis.station[1].staxyz, east),
                ]
            )
            y = np.array(
                [
                    np.inner(vis.station[0].staxyz, north),
                    np.inner(vis.station[1].staxyz, north),
                ]
            )
            ax.plot(
                x,
...
def plot_uv_size(oidata, maxvis=0.3, waveidx=80):
    ...
    ax.set_ylabel("v (m)")
    ax.set_title(
        r"$\lambda = %g$ $\mu$m" % (1e6 * oidata.vis[0].wavelength.eff_wave[waveidx])
    )
